...
// ✅ Glow generator — creates smooth, vibrant, and consistent glow around circles
...
const hierarchyData = [
    ...
    { id: 7, name: "At Lokhandwala Hills", name2: "Residential Banglows", icon: Home, info: "All Enterior Works", level: 3, bgColor: "bg-pink-600", glowColor: "244, 114, 182", iconColor: "At Pune" },
...
// ✅ Node Component (with glowing circle)
const Node = ({ data, onHover, onHoverEnd }) => {
    const Icon = data.icon;
    const isRoot = data.level === 1;

    const sizeClass = isRoot ? 'w-24 h-24 text-4xl' : 'w-24 h-24 text-4xl';
    const nameClass = isRoot ? 'text-lg mt-2' : 'text-sm mt-1';
    const marginClass = isRoot ? 'mb-10' : 'mb-8';

    return (
        <motion.div
            className={`flex flex-col items-center cursor-pointer p-2 ${marginClass}`}
        ...
        >
            <div
                style={getGlowStyle(data.glowColor)} // ✅ Apply perfect glowing shadow
                className={`
          rounded-full 
          ${data.bgColor} 
          ${data.iconColor} 
        ...
        `}
            >
                <Icon size={isRoot ? 40 : 32} className="drop-shadow-lg" />
            </div>
            <div className={`font-semibold text-white ${nameClass}`}>
                {data.name}
            </div>
        </motion.div>
    );
...
const Hierarchy = () => {
    ...
    return (
        <div className="p-4 md:p-10 min-h-screen bg-black">
            <h1 className="text-4xl font-bold text-center mb-12 text-yellow-400">
                {/* 🚀 Circular Hierarchy Map */}
                Places Where We Already Worked
            </h1>

            {/* Grid Layout */}
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-y-10 md:gap-y-12 gap-x-4 md:gap-x-8 max-w-7xl mx-auto">
